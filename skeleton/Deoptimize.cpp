	...
	for (auto& I : B) {
	  if (auto* instr = dyn_cast<BinaryOperator>(&I)) {
	    if(instr->getOpcode() == Instruction::Mul){
	      ...
	      Type *intype = IntegerType::getInt32Ty(cntxt);
